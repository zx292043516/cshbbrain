 ...
 * <li>修改日期：
 */
public class WebSocketCoder implements CoderHandler {
	...
	public void codeVersion6(Client sockector, Response msg){		
		...
		// todo list
		headers[0] = MessageFrame.FIN;// 需要调整
		headers[0] |= messageFrame.getRsv1() | messageFrame.getRsv2() | messageFrame.getRsv3() | MessageFrame.TXT;
		headers[1] = 0;
		...
		if (messageFrame.getPayloadLen() == MessageFrame.HAS_EXTEND_DATA) {// 处理数据长度为126位的情况
			msg.appendBytes(CoderUtils.shortToByte(messageFrame.getPayloadLenExtended()));
		} else if (messageFrame.getPayloadLen() == MessageFrame.HAS_EXTEND_DATA_CONTINUE) {// 处理数据长度为127位的情况
